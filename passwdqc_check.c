    ...
    const char *needle, const char *needle_original, unsigned int flags)
{
	...
	for (j = params->match_length; i + j <= length; j++) {
		int bias = 0;
		for (p = haystack; *p; p++)
		if (*p == needle[i] && !strncmp(p + 1, &needle[i + 1], j - 1)) { /* or memcmp() */
    ...
    const char *unified, const char *reversed, const char *original)
{
	...
	for (i = 0; i < sizeof(seq) / sizeof(seq[0]); i++) {
		...
		if (!seq_i)
			goto out;
		if (is_based(params, seq_i, seq_i, unified, original, F_SEQ) ||
		    is_based(params, seq_i, seq_i, reversed, original, F_SEQ|F_REV)) {
			clean(seq_i);
			reason = strstr(seq[i], "pass") ? REASON_WORD : REASON_SEQ;
			goto out;
		}
		clean(seq_i);
	}
